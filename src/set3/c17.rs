...
impl Oracle {
    ...
    fn new() -> Self {
        ...
        let plaintexts = Self::PLAINTEXTS
            .iter()
    ...
    fn encrypt(&mut self) -> Vec<u8> {
        ...
        [
            &self.iv[..],
            &self.aes.encrypt(plaintext, Input::Iv(self.iv)).unwrap(),
        ]
        .concat()
    }
        ...
        for n in 0..Aes::BLOCK_SIZE {
            ...
            loop {
                ...
                if oracle.decrypt(&block, chosen_ciphertext) {
                    ...
                    let original_byte =
                        chosen_ciphertext[index] ^ ((n + 1) as u8) ^ prev_block[index];
