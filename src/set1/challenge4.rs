...
fn find_plaintext<P: AsRef<Path>>(filename: P) -> Vec<u8> {
    ...
    let ciphertexts: Vec<Vec<u8>> = lines.map(|hex| Hex::try_from(hex.as_str()).unwrap().to_bytes()).collect();
...
fn verify() {
    ...
    assert_eq!(String::from_utf8(plaintext).unwrap().as_str(), "Now that the party is jumping\n")
}
